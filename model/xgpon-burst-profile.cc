...
XgponBurstProfile::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::XgponBurstProfile")
    ...
    .AddAttribute ("PsbuPreambleLength",
      "Length of the preamble for PSBu (unit: byte),",
      UintegerValue (PSBU_PREAMBLE_DEFAULT_LEN),
      MakeUintegerAccessor (&XgponBurstProfile::m_preambleLen),
      MakeUintegerChecker<uint8_t> ())
    .AddAttribute ("PsbuDelimiterLength",
      "Length of the delimiter for PSBu (unit: byte)",
      UintegerValue (PSBU_DELIMITER_DEFAULT_LEN),
      MakeUintegerAccessor (&XgponBurstProfile::m_delimiterLen),
      MakeUintegerChecker<uint8_t> ())
