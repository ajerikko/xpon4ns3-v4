...
public:
  ...
  const static uint32_t XGTC_USBURST_HEADERTRAILER=2;          //xgtc-us-burst header (w/o ploam)+trailer size; unit: in blocks (or BASE GRANT SIZE)
private:
...
public:
   ...
   */
  void Initialize(uint16_t onuId, bool ploam, const Ptr<XgponBurstProfile>& profile, uint16_t guardTime, uint16_t dataBlockSize, uint16_t fecBlockSize, uint8_t baseGrantSize);
   ...
   */
  void AddOneNewBwAlloc(const Ptr<XgponXgtcBwAllocation>& bwAlloc, const Ptr<XgponTcontOlt>& tcontOlt, uint8_t baseGrantSize); //ja:update:xgsponv5, converting values to Bytes, instead of to Blocks, to avoid losing sub interger values when dividing
   ...
   * \param extraGrantSize The amount of base grant size (4/16 bytes) to add to the bwAlloc
   */
...
private:
  ...
  uint8_t  m_gapPhyOverhead;       //GAP + PREAMBLE + DELIMITER. unit: BASE_XGPON/XGSPON_GRANT_SIZE
  ...
  uint16_t m_dataBlockSize;        //used to calculate burst size when FEC is used. unit: BASE_XGPON/XGSPON_GRANT_SIZE
  uint16_t m_fecBlockSize;  
  uint32_t m_headerTrailerDataSize;//XGTC header (ploam included if exist) + BwAllocations. unit: BASE_XGPON/XGSPON_GRANT_SIZE
  //updated after each XgponXgtcBwAllocation is added
  uint32_t m_finalBurstSize;       //final size (all stuffs after FEC if FEC is used). unit: BASE_XGPON/XGSPON_GRANT_SIZE 
