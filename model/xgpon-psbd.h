...
private:
  ...
  uint64_t  m_ponId;       //51bits - PON-ID structure is 8 Bytes (64bits) long with the first 51bits as PON-ID and the other 13bits as HEC for PON-ID
...
{
  m_ponId = ponId & 0x0007ffffffffffff;//this ensures that the ponId is converted to 51bits (0x0007ff..... has the first 13 bits as 0 to allow space for PON-ID HEC)
}
inline uint64_t 
