...
protected:  
  ...
  uint64_t m_maxPackets;           //Queue size in packet
  uint64_t m_maxBytes;             //Queue size in byte


  //current queue status
  uint64_t m_nPackets;             //The amount of data (unit: packet) in queue
  uint64_t m_nBytes;               //The amount of data (unit: byte) in queue
  uint64_t m_nBlocks4Scheduling;    //the amount of data (unit: blocks of - word in XGPON / 4-word in XGSPON) in queue with the assumption that each packet is padded if needed, ja:update:xgspon

  //segmentation related variables
  Ptr<Packet> m_remainingSegment;
  
  std::vector<double> m_allPacket; // vector to store a number of metadata (index, time) of the packet queued
...
{
  return (m_remainingSegment!=nullptr);
}
...
{
  if(m_remainingSegment!=nullptr) return CalculatePacketSize4Scheduling(m_remainingSegment->GetSize());
  else return 0;
}
...
XgponQueue::CalculatePacketSize4Scheduling (uint32_t pktSizeInByte)
{
  ...
  sizeInByte += XgponXgemHeader::XGPON_XGEM_HEADER_LENGTH;    //Xgem header overhead in word
	...
	uint32_t sizeInBlocks = sizeInByte/m_baseGrantSize;
	if(sizeInBlocks*m_baseGrantSize < sizeInByte){//ja:update:xgsponv5, a dirty way of ceiling the values for blocks
		sizeInBlocks +=1;
	}
  return sizeInBlocks;
}
