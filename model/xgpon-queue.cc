...
XgponQueue::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::XgponQueue")
    ...
    .AddAttribute ("Mode", 
                   ...
                   MakeEnumAccessor<XgponQueue::XgponQueueMode>(&XgponQueue::SetMode), //suitable for ns3.41
                   //MakeEnumAccessor(&XgponQueue::SetMode), //suitable for ns3.40 (l4s compatible version)
    ...
    .AddAttribute("BaseGrantSize",
                  ...
                  MakeUintegerAccessor(&XgponQueue::m_baseGrantSize),
...
XgponQueue::XgponQueue() : 
  ...
  m_baseGrantSize(BASE_GRANT_SIZE_XGPON)
{
  ...
  if (retval)
    {
      ...
      uint32_t sizeBlocks = CalculatePacketSize4Scheduling(size);
      m_nBlocks4Scheduling += sizeBlocks;
...
{
  NS_ASSERT_MSG((m_remainingSegment ==nullptr), "Something is WRONG!!! Segments exist.");
