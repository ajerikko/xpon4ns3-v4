...
XgponOnuFramingEngine::ProduceXgtcUsBurst (XgponXgtcUsBurst& usBurst, const Ptr<XgponXgtcBwmap>& map, uint16_t first)
{
  ...
  int num = map->GetNumberOfBwAllocation();
	do
  {
  	int allocSizeInBytes = m_baseGrantSize*bwAlloc->GetGrantSize (); //unit: Bytes,  ja:update:xgspon, replaced allocSize with allocSizeInBytes
    uint16_t allocId = bwAlloc->GetAllocId ();
  	if(allocSizeInBytes >0){
  	  ...
  	  usBurst.AddUsAllocation(allocation);
		  if(bwAlloc->GetDbruFlag ()){  //buffer occupancy report if the dbru flag is set
  			Ptr<XgponXgtcDbru> dbr; 
	  		dbr = dbaEngine->GenerateStatusReport(allocId);//status report is sent along with the data, based on the allocation
			  allocSizeInBytes = allocSizeInBytes - 4;
  			allocation->SetDbru(dbr);
	  	}
    
      //Generate XGEM Frames to be transmitted and put them into UsAllocation
		  xgemEngine->GenerateFramesToTransmit(allocation->GetXgemFrames(), allocSizeInBytes, allocId); //ja:update:xgspon replaced allocSize with allocSizeInBytes
		}
