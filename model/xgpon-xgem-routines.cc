...
XgponXgemRoutines::GenerateXgemFrame (const Ptr<XgponNetDevice>& device, const Ptr<XgponConnectionSender>& conn, uint32_t maxLen, const Ptr<XgponKey>& key, uint8_t keyIndex, bool doSegmentation)
{
  ...
  if(sdu == nullptr) return 0;  //all packets had been transmitted.
  ...
  //std::cout << "\t\t\tactual XGEM frameSize: " << frameSize << ", sduSize: " << sduSize << ", maxLen: " << maxLen << std::endl; //ja:update:xgspon
  if(frameSize <= maxLen)
  {
...
XgponXgemRoutines::CreateXgemFrameWithData(const Ptr<Packet>& sdu, uint16_t portId, const Ptr<XgponKey>& key, uint8_t keyIndex, bool lastFrame)
{
  ...
  //std::cout << "at Time: " << Simulator::Now().GetNanoSeconds() << ", sdu Size: " << sdu->GetSize() << ", keyIndex: " << keyIndex << ", portId: " << portId << ", lastFrame: " << lastFrame << std::endl; //ja:update:xgspon
