...
XgponPhy::CalculateUsPhyFrameSize ( )
{
  ...
  tmp = (((uint64_t)m_usLinkRate * (uint64_t)m_dsFrameSlotSize) / 1000000000)/m_baseGrantSize;  //ja:update:xgsponv5, a mistake from the earlier xgspon implementation, but one that did not impact the voice results because of low traffic rate; but the return value has a unit of Blocks of Word/4-Bytes (XGPON) or Blocks of 16-Bytes (XGSPON)
