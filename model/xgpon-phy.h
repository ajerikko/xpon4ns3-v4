...
class XgponPhy : public Object
{
  ...
  const static uint32_t XGPON1_MINIMUM_GUARD_TIME = 2;          //unit: word (4 bytes per word) minimum guard time 64bits

  const static uint32_t XGPON1_FEC_BLOCK_SIZE = 248;            //unit: byte
  const static uint32_t XGPON1_DS_FEC_DATA_SIZE = 216;          //unit: byte, 216 Bytes for FEC Data Bytes and 32 Bytes for parity
  const static uint32_t XGPON1_US_FEC_DATA_SIZE = 232;          //unit: byte, 232 Bytes for FEC Data Bytes and 16 Bytes for parity

	const static uint8_t BASE_GRANT_SIZE_XGPON = 4;         //unit: Bytes, ja:update:xgspo 
...
public:
  ...
  /**
   * \brief Get the length of upstream phy frame. Unit: 4-Bytes (XGPON) or 16-Bytes (XGSPON), ja:update:xgsponv5. used by DBA engine  
   * \return the length of upstream phy frame in word
   */
  uint32_t GetUsPhyFrameSizeInBlocks ( );
...
protected:
  ...
  uint32_t m_usMinimalGuardTime;  //the size of minimum guard time for upstream bursts in word (4 byte).
...
private:
  ...
  uint32_t CalculateUsPhyFrameSize ( );
	...
	uint8_t m_baseGrantSize; //unit: Bytes, ja:update:xgspon5
...
inline uint32_t 
XgponPhy::GetUsPhyFrameSizeInBlocks ( )
{
