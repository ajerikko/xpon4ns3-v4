...
public:
   ...
   */
  void SendSduToUpperLayer (const Ptr<Packet>& sdu, uint16_t tcontType, uint16_t senderId, uint16_t receiverId);
...
{
  NS_ASSERT_MSG ((m_commonPhy!=nullptr), "phy has not been set yet.");
  return m_commonPhy;
}
...
{
  NS_ASSERT_MSG((m_qosParameters!=nullptr), "QoS parameter has not been set yet.");
