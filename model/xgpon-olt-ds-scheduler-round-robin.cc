...
XgponOltDsSchedulerRoundRobin::SelectConnToServe (uint32_t* amountToServe)
{
  ...
  if(m_startFrame)
  {
    ...
    {
      *amountToServe = lastConn->GetFragBufOccupancy4Scheduling () * m_baseGrantSize; //ja:update:xgspon
      return lastConn;
    }
  }
  ...
  uint32_t dataInQueue = conn->GetBufOccupancy4Scheduling () * m_baseGrantSize; //ja:update:xgspon
  if(dataInQueue < m_maxServiceSize)  *amountToServe = dataInQueue;
  else *amountToServe = m_maxServiceSize;

  //std::cout << "at time: " << Simulator::Now().GetNanoSeconds() << ", amountToServe: " << *amountToServe << ", conn: " << conn << std::endl;//ja:update:xgspon
  return conn;
}
