...
public:
   ...
   * \return the amount of data (padded + xgem header). unit: blocks of word (XGPON) or blocks of 4-words (XGSPON)
   */
   ...
   * \return the size of the segment (padded + xgem header). Unit: blocks of word (XGPON) or bloacks of 4-words (XGSPON)
   */
...
{
  NS_ASSERT_MSG((m_txQueue!=nullptr), "Tx-queue has not been set yet.");
  return m_txQueue;
}
