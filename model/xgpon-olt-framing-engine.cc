...
XgponOltFramingEngine::ProduceXgtcDsFrame (XgponXgtcDsFrame& xgtcDsFrame)
{
  ...
  //std::cout << "dsFrame frame size: " << (m_device->GetXgponPhy())->GetXgtcDsFrameSize ( ) << ", payload Len : " << payloadLen << std::endl;//ja:update:xgspon
...
void 
XgponOltFramingEngine::ParseXgtcUpstreamBurst (XgponXgtcUsBurst& burst, uint64_t nowNano)//ja:update:xgsponv5
{
  NS_LOG_FUNCTION(this);
//  uint64_t nowNano = Simulator::Now().GetNanoSeconds();
  ...
  NS_ASSERT_MSG((bwmap!=nullptr), "Cannot find the corresponding Bwmap of this upstream burst.");
  ...
  for(int i=first, j=0; j<num; i++, j++)
  {
    ...
    NS_ASSERT_MSG(((tcontOlt!=nullptr) && (tcontOlt->GetOnuId()==onuId)), "Cannot find the corresponding Bwmap of this upstream burst.");

    //queue status report
    if(bwAlloc->GetDbruFlag ()) dbaEngine->ReceiveStatusReport(alloc->GetDbru(), onuId, allocId, nowNano); 
