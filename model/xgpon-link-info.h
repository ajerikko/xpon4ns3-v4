...
{
  NS_ASSERT_MSG((m_activeDsKeys[m_curDsKeyIndex]!=nullptr), "Try to use one null XgponKey.");
  return m_activeDsKeys[m_curDsKeyIndex];
}
inline const Ptr<XgponKey>& 
XgponLinkInfo::GetCurrentUsKey ( ) const
{
  NS_ASSERT_MSG((m_activeUsKeys[m_curUsKeyIndex]!=nullptr), "Try to use one null XgponKey.");
  return m_activeUsKeys[m_curUsKeyIndex];
}
inline const Ptr<XgponKey>& 
XgponLinkInfo::GetDsKeyByIndex (uint8_t index) const
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_KEYS) && (m_activeDsKeys[index]!=nullptr)), "Try to use one null XgponKey.");
  return m_activeDsKeys[index];
}
inline const Ptr<XgponKey>& 
XgponLinkInfo::GetUsKeyByIndex (uint8_t index) const
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_KEYS) && (m_activeUsKeys[index]!=nullptr)), "Try to use one null XgponKey.");
  return m_activeUsKeys[index];
}
inline void 
XgponLinkInfo::SetCurrentDsKeyIndex (uint8_t index)
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_KEYS) && (m_activeDsKeys[index]!=nullptr)), "Key index is too large.");
  m_curDsKeyIndex = index;
}
inline void 
XgponLinkInfo::SetCurrentUsKeyIndex (uint8_t index)
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_KEYS) && (m_activeUsKeys[index]!=nullptr)), "Key index is too large.");
...
{
  NS_ASSERT_MSG((m_profiles[m_curProfileIndex]!=nullptr), "Try to use one null XgponKey.");
  return m_profiles[m_curProfileIndex];
}
inline const Ptr<XgponBurstProfile>& 
XgponLinkInfo::GetProfileByIndex (uint8_t index) const
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_PROFILES) && (m_profiles[index]!=nullptr)), "Try to use one null XgponKey.");
  return m_profiles[index];
}
inline void 
XgponLinkInfo::SetCurrentProfileIndex (uint8_t index)
{
  NS_ASSERT_MSG(((index<MAX_NUMBER_OF_PROFILES) && (m_profiles[index]!=nullptr)), "Profile index is too large.");
