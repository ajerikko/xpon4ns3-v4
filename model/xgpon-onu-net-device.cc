...
XgponOnuNetDevice::ReceivePonFrameFromChannel (const Ptr<PonFrame>& frame)
{
  ...
  //std::cout << "time now is: " << m_stat.m_currentTime << std::endl;
...
XgponOnuNetDevice::DoSend (const Ptr<Packet>& packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  packet->PeekHeader(ipHeader);
	...
	NS_LOG_FUNCTION (this << ipHeader.GetSource() << dstAddress << packet << protocolNumber << dest);

  m_tcontType = ipHeader.GetTos() >> 2; //ja l4sv1, tcont type is derived from the TOS field of the header and after adjusting for the 2-bits of ECN in the IP tos field; if a suitable value is set (1 to 4), the appropriate tcont type is used
  //std::cout << "m_tcontType after adjusting: " << m_tcontType << std::endl;
  ...
  NS_ASSERT_MSG(((m_tcontType <= 4) & (m_tcontType >= 0)), "A suitable TOS value (1-4) is not set for the traffic end points");
...
XgponOnuNetDevice::ProduceAndTransmitUsBurst (const Ptr<XgponXgtcBwmap>& map, uint16_t first)
{
  NS_LOG_FUNCTION (this);
    ...
    NS_ASSERT_MSG((bwAlloc!=nullptr), "Cannot find the XgponXgtcBwAllocation used to produce this burst!!!");
