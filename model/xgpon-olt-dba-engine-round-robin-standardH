...
{
public:
  const static uint32_t XGPON1_US_PER_SERVICE_MAX_SIZE=9720;    //9718K blocks of words (around 40KBytes) or 4-words (around 160KBytes) to let one flow to fuly utilize XGPON or XGSPON respectively) ja:update:xgspon, ja:update:xgsponv5, 9718 to 9720 to be in sync with other values 
...
private:
  ...
  uint16_t m_lastSchTcontIndexForFrame;   //the index in this tcont-type list that are scheduled most recently.
	uint16_t m_lastSchTcontIndexForCycle; 	//ja:update:xgsponv5, an additional index to keep track of indexes across DBA cycles
	bool m_dbaCycleStart;
  Ptr<XgponTcontOlt> m_nullTcont;      	// Pointer used to return a null T-CON
