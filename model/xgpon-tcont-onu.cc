...
XgponTcontOnu::PrepareBufOccupancyReport ( )
{
  ...
  AddNewBufOccupancyReport(dbr);
	...
	//std::cout << "at_time," << nanoNow << ",nanoseconds,generateSR,setBufOccupancy," << bufOccupancy*16 << ",Bytes(forXGSPON),allocId," << GetAllocId() << std::endl;
...
{
  NS_ASSERT_MSG((m_usScheduler!=nullptr), "Upstream scheduler is not set for this ALLOC-ID!!!");
